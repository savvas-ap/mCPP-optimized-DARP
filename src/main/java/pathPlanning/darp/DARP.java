...
public class DARP {
    ...
    public void constructAssignmentM() {
        ...
        DesireableAssign = new double[nr];
        for (int i = 0; i < nr; i++) {
            DesireableAssign[i] = effectiveSize * Rportions[i];
            if (DesireableAssign[i] != (int) DesireableAssign[i] && termThr!=1){
                termThr = 1;
            }
        }
        ...
        while (termThr <= discr && !success && !canceled) {
            ...
            while (iter <= maxIter && !canceled) {
                ...
                for (int r = 0; r < nr; r++) {
                    ...
                    plainErrors[r] = ArrayOfElements[r] / (DesireableAssign[r] * nr);
